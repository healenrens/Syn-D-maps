...
def _merge_road_boundaries(map_data: Dict[str, List[List]]) -> Dict[str, List[List]]:
        processed_data = {k: v.copy() for k, v in map_data.items()}
        
        if 'road_boundary' not in map_data:
            return processed_data
            
        boundaries = map_data['road_boundary']
        merged_boundaries = []
        used = set()

        for i, line1 in enumerate(boundaries):
            if i in used:
                continue
            ...
            while True:
                best_angle = np.pi 
                best_idx = None
                start_point = current_line[0]
                end_point = np.array(current_line[-1])
                now_distance = np.linalg.norm(start_point - end_point)
                direction1 = np.array(current_line[-1]) - np.array(current_line[-2])
                
                for j, line2 in enumerate(boundaries):
                    if j in used:
                        continue

                    if np.allclose(end_point, line2[0], atol=1e-6):
                        direction2 = np.array(line2[1]) - np.array(line2[0])
 
                        _add_distance = np.linalg.norm(np.asarray(start_point) - np.asarray(line2[1]))
                        if _add_distance < now_distance*0.98:
                            break
                        ...
                        dir1_normalized = direction1 / norm1
                        dir2_normalized = direction2 / norm2
                        cross_product = np.cross(dir1_normalized, dir2_normalized)
                ...
                if best_idx is not None and best_angle < np.pi/12:
                    next_line = boundaries[best_idx]
                    current_line.extend(next_line[1:]) 
                    used.add(best_idx)
                else:
                    break
            
            merged_boundaries.append(current_line)
        
        processed_data['road_boundary'] = merged_boundaries
        return processed_data
...
def bezier_deform(
                    ...
                    num_segments: int = 200,keep_original=False) -> Dict[str, List[np.ndarray]]:
        distance_thresholds = {
            'high': 1.5,    # Rh = 1.5m
            'medium': 1.0,  # Rm = 1.0m
            'low': 0.5      # Rl = 0.5m
        }
        processed_map_data = _merge_road_boundaries(map_data)
        elements =_convert_to_elements(processed_map_data)
        ...
        def get_bezier_point(control_points: np.ndarray, t: float) -> np.ndarray:
            ...
            return point
        def get_curve_points(control_points: np.ndarray, num_segments: int = 200) -> np.ndarray:

            t = np.linspace(0, 1, num_segments)
            return np.array([get_bezier_point(control_points, ti) for ti in t])
        def find_closest_point(point: np.ndarray, curve_points: np.ndarray) -> Tuple[np.ndarray, float]:
            distances = np.linalg.norm(curve_points - point, axis=1)
            idx = np.argmin(distances)
            t = idx / (len(curve_points) - 1)
            return curve_points[idx], t
        def _convert_to_dict(elements: List[MapElement]) -> Dict[str, List[np.ndarray]]:
            ...
            return result
        weight = random.uniform(0.05, 0.10)
        deformed_elements = []
        set_trend = False
        for element in elements:
            ...
            if not set_trend:
                set_trend = True
                trend = -1 if np.random.random() < 0.5 else 1
            normal *= trend
            

            length = np.linalg.norm(line_vector)
            offset = normal * length * weight
            line_distance = np.linalg.norm(line_vector)
            p1 = start + line_vector / 3 + offset
            p2 = start + 2 * line_vector / 3 + offset
            control_points = np.array([start, p1, p2, end])
            
           
            curve_points = get_curve_points(control_points, int(num_segments*(line_distance/10)))
            
           
            deformed_points = []
            if keep_original:
                for point in element.points:
                    closest_point, t = find_closest_point(point, curve_points)
                    deformed_point = get_bezier_point(control_points, t)
                    deformed_points.append(deformed_point)

            else:
                deformed_points = curve_points
            
            deformed_elements.append(MapElement(
                points=np.array(deformed_points),
...
def modify_lane_groups(
        ...
        original_boundary: Dict[str, Union[str, np.ndarray]],) -> Tuple[
    ...
    group.pop('lane_number')
    group.pop('direction')
    group.pop('is_center_in')
    group.pop('c')
    original_key = list(group.keys())
    transformer_group = {}
    for key in group:
        transformer_group[key] = []
        for instance_points in group[key]:
            transformer_group[key].append(instance_points['points'].tolist())
    transformer_group = bezier_deform(transformer_group, target_ED=1)
    # print(transformer_group)
    new_group = {}
    for key in transformer_group:
        new_group[key] = []
        original_key.remove(key)
        for i, instance_points in enumerate(transformer_group[key]):
            new_group[key].append({'token': f'NULL_' + key + '{i}', 'points': np.array(instance_points)})
    for key in original_key:
        new_group[key] = group[key]
    lane_groups[group_index] = new_group
    modified_groups = lane_groups
    road_boundary = remain_boundary
    change_number = len(new_group['lane_divider']) + len(new_group['road_divider']) + 2

    return modified_groups, road_boundary, True, change_number


def bezier_warp(input_data_: Dict[str, List[List[List[float]]]], max_changes: float):
    ...
    for index in range(group_number):
        ...
        if change_success:
            #print(index, group_change_number)
