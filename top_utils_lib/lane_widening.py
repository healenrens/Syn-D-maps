...
def scale_lane(lane_points: np.ndarray, scale_factor: float, reference_center: np.ndarray = None) -> np.ndarray:
    ...
    return np.array(scaled_lane), np.mean(norm_offset) 
...
def lane_widening(input_data_: Dict[str, List[List[List[float]]]], max_changes: float):
