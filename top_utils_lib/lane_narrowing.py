...
def scale_lane(lane_points: np.ndarray, scale_factor: float, reference_center: np.ndarray = None) -> np.ndarray:
    ...
    for point in lane_points:
        ...
        norm_offset.append((abs(scaled_offset) - abs(offset)))
        scaled_lane.append(scaled_point)
    return np.array(scaled_lane), np.mean(norm_offset)  # why not be negitave
...
def calculate_centerline(points: np.ndarray) -> tuple[np.ndarray, np.ndarray]:

    center = np.mean(points, axis=0)

    direction = calculate_lane_direction(points)  

    projected_points = np.dot(points - center, direction)
    max_proj = np.max(projected_points)
    min_proj = np.min(projected_points)
...
def modify_lane_groups(
        ...
        operation: str,
        scale_factor: float = config.scale) -> Tuple[
    ...
    change_number = 0
    if operation == "road_contraction":
        if len(group['lane']) > 1:
            random_index = random.sample(range(0, len(group['lane']) - 1), 1)
        else:
            random_index = [0]
        idx_expand = random_index[0]
        lane = group['lane'][idx_expand]
        lane_points = lane['points']
        expand_points, move_distance = scale_lane(lane_points, -scale_factor + 1)
        lane['points'] = expand_points
        this_center_line = calculate_centerline(expand_points)

        ignore_token = [lane['token']]
        groups, road_boundary, total_change = push_elements_from_center(this_center_line, lane_groups,
                                                                             original_boundary, ignore_token, move_distance)
        modified_groups = groups
        change_number = total_change
    return modified_groups, road_boundary, True, change_number


def lane_narrowing(input_data_: Dict[str, List[List[List[float]]]], max_changes: float):
    ...
    for index in range(group_number):
        if group_area[index] < rate_average:
            continue
        gc.collect()
